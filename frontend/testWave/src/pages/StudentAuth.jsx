...
const StudentAuth = () => {
  ...
  const handleVerifySignupOtp = async () => {
    ...
    try {
      ...
      sessionStorage.setItem("studentEmail", formData.email); // Set email in session
  ...
  return (
    ...
    <div div className="container p-4 rounded shadow bg-white" style={{ maxWidth: "500px", opacity: 0.95 }}>
      <h3 className="text-center mb-4">Student Portal - Testwave</h3>

      <div className="d-flex justify-content-around mb-4">
        <button className={`btn btn-outline-primary ${view === "login" ? "active" : ""}`} onClick={() => setView("login")}>Login</button>
        <button className={`btn btn-outline-success ${view === "signup" ? "active" : ""}`} onClick={() => setView("signup")}>Signup</button>
        <button className={`btn btn-outline-warning ${view === "forgot" ? "active" : ""}`} onClick={() => setView("forgot")}>Forgot Password</button>
      </div>
